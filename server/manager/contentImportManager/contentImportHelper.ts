...
import { logger, enableLogger } from "@project-sunbird/ext-framework-server/logger";
import { debounceTime, throttleTime } from "rxjs/operators";
import { logSync, logAsync } from "./../../logDecoretor";
...
class ImportWorker {
  ...
  public async extractEcar() {
    try {
      ...
      // delete temp content folder which has manifest.json
...
process.on("message", (data) => {
  logger.info("------got data------", data.message);
  initFramework(() => {
...
let frameworkInit = false;
import * as express from "express";
import { frameworkAPI } from "@project-sunbird/ext-framework-server/api";
const subApp = express();
const NS_PER_SEC = 1e9;
const initFramework = (cb) => {
  ...
  .then(() => {
    ...
    logger.info(`-----inti child framework took ${endTime} nanoseconds----`);
    frameworkInit = true;
    cb();
  })
  .catch((error: any) => {
...
const frameworkConfig = {
  logBasePath: path.join(__dirname, './../../../../'),
  db: {
    cassandra: {
